...
fn parse_value_u8() {
    assert_eq!(
        parse_value("255", &ValueType::U8).unwrap(),
...
fn parse_value_i8() {
    assert_eq!(
        parse_value("-128", &ValueType::I8).unwrap(),
...
fn parse_value_u16() {
    assert_eq!(
        parse_value("65535", &ValueType::U16).unwrap(),
...
fn parse_value_i16() {
    assert_eq!(
        parse_value("-1", &ValueType::I16).unwrap(),
        MetadataValue::I16(-1)
    );
}
#[test]
...
fn parse_value_i32() {
    assert_eq!(
        parse_value("-42", &ValueType::I32).unwrap(),
        MetadataValue::I32(-42)
    );
}
#[test]
fn parse_value_u64() {
    assert_eq!(
        parse_value("0", &ValueType::U64).unwrap(),
        MetadataValue::U64(0)
    );
}
#[test]
fn parse_value_i64() {
    assert_eq!(
        parse_value("-1", &ValueType::I64).unwrap(),
        MetadataValue::I64(-1)
    );
}
#[test]
fn parse_value_f32() {
    assert_eq!(
        parse_value("1.5", &ValueType::F32).unwrap(),
        MetadataValue::F32(1.5)
    );
}
#[test]
fn parse_value_f64() {
    assert_eq!(
        parse_value("2.5", &ValueType::F64).unwrap(),
...
fn set_existing_key_roundtrip() {
    ...
    assert_eq!(val, &MetadataValue::String("new-model-name".to_string()));
}
...
fn set_backup_renames_existing_output() {
    ...
    assert!(
        bak_path.exists(),
        "previous output must be preserved as .bak"
    );
