...
fn export_json_contains_all_three_keys() {
    ...
    assert!(
        obj.contains_key("general.architecture"),
        "missing general.architecture"
    );
    assert!(obj.contains_key("general.name"), "missing general.name");
    assert!(
        obj.contains_key("llm.context_length"),
        "missing llm.context_length"
    );
}
...
fn export_markdown_starts_with_header() {
    ...
    assert!(
        output.starts_with("| Key |"),
        "must start with markdown table header"
    );
}
...
fn export_markdown_contains_values() {
    ...
    assert!(
        output.contains("test-arch"),
        "must include architecture value"
    );
...
fn export_csv_starts_with_header() {
    ...
    assert!(
        output.starts_with("key,type,value\n"),
